<script lang="ts">
	...
	const sendOTP = async (e?: Event) => {
		...
		errorMessage = '';
		try {
			const response = await apiService.post('/auth/login', { nip: nip });
			if (!response.ok) {
			...
			errorMessage = 'Terjadi kesalahan saat mengirim OTP';
			isLoading = false;
			return;
		}
	...
	const handleLogin = async (e?: Event) => {
		...
		try {
			const response = await apiService.post('/auth/verify-otp', { nip: nip, otp: otp });
			if (!response.ok) {
				errorMessage = await response.text();
				isLoading = false;
				return;
			...
			errorMessage = 'Terjadi kesalahan saat memverifikasi OTP';
			isLoading = false;
			return;
		}
				...
				<div class="hidden w-full bg-linear-to-br from-primary to-secondary p-8 md:block md:w-2/5">
					...
					>
						<div class="absolute inset-0 flex items-center justify-center">
							<div class="h-64 w-64 rounded-full bg-white opacity-10"></div>
						</div>
						<div
						...
						<div class="relative z-10">
							<div class="bg-opacity-20 mb-6 inline-block rounded-full bg-white p-4">
								<svg
							...
							<h2 class="mb-2 text-2xl font-bold text-white">Sistem Pendataan</h2>
							<p class="text-opacity-80 text-white">Situs Keagamaan KUA Kecamatan Ciemas</p>
						</div>
					</div>
				</div>
							...
							{#if isOtpSent}
								<div class="mb-6">
									<label class="label" for="otp">
										<span class="label-text font-medium text-base-content">Kode OTP</span>
									</label>
									<input
										id="otp"
										type="text"
									...
									<div class="label">
										<span class="label-text-alt text-base-content/60"
											>Kode telah dikirim ke nomor terdaftar</span
										>
									</div>
								</div>
							{/if}
